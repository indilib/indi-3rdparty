...
/*
    Coordinate:
        (a) Functions with coordinate parameters, such as Tscam_put_Roi, Tscam_put_AEAuxRect, etc., the coordinate is always relative to the original resolution,
...
#define TSCAM_FLAG_40GIGE               0x0001000000000000  /* 40 Gigabit GigE */
...
#define TSCAM_FLAG_USB32                0x0400000000000000  /* USB 3.2 Gen 2 */
#define TSCAM_FLAG_USB32_OVER_USB30     0x0800000000000000  /* USB 3.2 Gen 2 camera connected to usb3.0 port */
#define TSCAM_FLAG_LINESCAN             0x1000000000000000  /* line scan camera */
#define TSCAM_FLAG_25GIGE               0x2000000000000000  /* 2.5 Gigabit GigE */
#define TSCAM_FLAG_RAW14PACK            0x4000000000000000  /* pixel format, RAW 14bits packed */
...
#define TSCAM_ANTIBLOOMING_MIN          0       /* Anti Blooming */
#define TSCAM_GVCP_RETRY_DEF            4       /* GVCP Retry */
#define TSCAM_GVCP_RETRY_MIN            2
#define TSCAM_GVCP_RETRY_MAX            20
#define TSCAM_GVCP_TIMEOUT_DEF          40      /* GVCP Timeout */
#define TSCAM_GVCP_TIMEOUT_MIN          20
#define TSCAM_GVCP_TIMEOUT_MAX          200
#define TSCAM_GVSP_WAIT_PERCENT_DEF     1       /* GVSP wait percent */
#define TSCAM_GVSP_WAIT_PERCENT_MIN     0
#define TSCAM_GVSP_WAIT_PERCENT_MAX     100
#define TSCAM_FRONTEND_MAX              1024    /* frontend frame buffer deque length */
#define TSCAM_FRONTEND_DEF              4
#define TSCAM_FRONTEND_MIN              2
#define TSCAM_BACKEND_MAX               1024    /* backend frame buffer deque length */
#define TSCAM_BACKEND_DEF               3
#define TSCAM_BACKEND_MIN               2
#define TSCAM_HEAT_MIN                  0       /* Heat */
#define TSCAM_LANE_MIN                  0       /* Lane */
#define TSCAM_FAN_MIN                   0       /* Fan */
...
#endif
    const TscamModelV2* model;              /* Functionally equivalent to a global constant, remaining valid and unchanged throughout the lifetime of the process */
...
*/
#if defined(_WIN32)
TSCAM_API(HRESULT)  Tscam_Load(HTscam h, const wchar_t* strPara);
#else
TSCAM_API(HRESULT)  Tscam_Load(HTscam h, const char* strPara);
#endif
...
#define TSCAM_EVENT_FRONT_OVERFLOW    0x0010    /* front buffer overflow */
#define TSCAM_EVENT_BACK_OVERFLOW     0x0011    /* back buffer overflow */
...
#if defined(_WIN32)
TSCAM_API(HRESULT)  Tscam_StartPullModeWithWndMsg(HTscam h, HWND hWnd, unsigned msgWnd);
#endif
...
typedef struct {
    ...
    unsigned char pixelFormat;      /* TSCAM_PIXELFORMAT_xxxx */
    unsigned char ergb;             /* see TSCAM_OPTION_RGB */
    unsigned char snapR;            /* see Tscam_SnapR */
    unsigned char infoVer;          /* TscamFrameInfo version >= 4 */
    unsigned reserved;
    unsigned strideRaw;             /* stride of RAW, 0 means = image width (no padding) */
    TscamFrameInfoV4* ptrInfo;
    void* snapCtx;
    void* ptrRaw;                   /* RAW, see TSCAM_OPTION_IMAGEPTRRAW */
    unsigned char* ptr8;
...
TSCAM_API(HRESULT)  Tscam_PullImagePtr(HTscam h, int bStill, TscamImagePtr* ptrImage);
...
/* After a frame buffer has been used, it must be returned to the SDK for reuse. Please note the following:
    ...
    (e) Frame buffers that have not been returned remain valid after Tscam_Stop. Frame buffers that have not been returned become invalid immediately after Tscam_Close.
*/
TSCAM_API(HRESULT)  Tscam_PushImagePtr(HTscam h, unsigned ptrId);
...
/* waitMS: The timeout interval, in milliseconds. If a nonzero value is specified, the function waits until the image is ok or the interval elapses.
            If waitMS is zero, the function does not enter a wait state if the image is not available; it always returns immediately; this is equal to Tscam_PullImagePtr.
*/
TSCAM_API(HRESULT)  Tscam_WaitImagePtr(HTscam h, unsigned waitMS, int bStill, TscamImagePtr* ptrImage);

/*
    waitMS: The timeout interval, in milliseconds. If a nonzero value is specified, the function waits until the image is ok or the interval elapses.
             If waitMS is zero, the function does not enter a wait state if the image is not available; it always returns immediately; this is equal to Tscam_PullImageV4.
    bStill: to pull still image, set to 1, otherwise 0
...
TSCAM_API(HRESULT)  Tscam_WaitImageV4(HTscam h, unsigned waitMS, void* pImageData, int bStill, int bits, int rowPitch, TscamFrameInfoV4* pInfo);
TSCAM_API(HRESULT)  Tscam_PullImageV3(HTscam h, void* pImageData, int bStill, int bits, int rowPitch, TscamFrameInfoV3* pInfo);
TSCAM_API(HRESULT)  Tscam_WaitImageV3(HTscam h, unsigned waitMS, void* pImageData, int bStill, int bits, int rowPitch, TscamFrameInfoV3* pInfo);
...
TSCAM_API(HRESULT)  Tscam_SnapV2(HTscam h, unsigned nResolutionIndex, unsigned nNumber, int eRGB, void* snapCtx);
...
*/
TSCAM_API(HRESULT)  Tscam_TriggerSyncV4(HTscam h, unsigned waitMS, void* pImageData, int bits, int rowPitch, TscamFrameInfoV4* pInfo);
TSCAM_API(HRESULT)  Tscam_TriggerSync(HTscam h, unsigned waitMS, void* pImageData, int bits, int rowPitch, TscamFrameInfoV3* pInfo);
...
*/
TSCAM_API(HRESULT)  Tscam_get_AutoExpoEnable(HTscam h, int* mode);
TSCAM_API(HRESULT)  Tscam_put_AutoExpoEnable(HTscam h, int mode);
...
/* Case insensitive */
TSCAM_API(HRESULT)  Tscam_put_Int(HTscam h, const char* strName, int iValue);
TSCAM_API(HRESULT)  Tscam_get_Int(HTscam h, const char* strName, int* piValue);
TSCAM_API(HRESULT)  Tscam_put_Str(HTscam h, const char* strName, const char* strValue);
/* strValue: output buffer (128 bytes are always sufficient) */
TSCAM_API(HRESULT)  Tscam_get_Str(HTscam h, const char* strName, char strValue[]);
TSCAM_API(HRESULT)  Tscam_get_Enum(HTscam h, const char* strName, int* pNumber, int arrValue[], const char* arrString[]);
TSCAM_API(HRESULT)  Tscam_get_StrPtr(HTscam h, const char* strName, const char** strValue);
...
#define TSCAM_OPTION_BLACKLEVEL             0x15       /* [RW] the black level refers to the baseline signal value output by an image sensor under no-light (completely dark) conditions.
...
#define TSCAM_OPTION_BINNING                0x17       /* [RW] digital binning
                                                                0x01: (no binning)
...
#define TSCAM_OPTION_CALLBACK_THREAD        0x30       /* [RW] dedicated thread for callback: 0 => disable, 1 => enable
                                                                 default: 1
                                                         */
...
#define TSCAM_OPTION_FPNC                   0x67       /* [RW] Fix Pattern Noise Correction
                                                             ...
                                                             get:
                                                                 ...
                                                                 ((val & 0xff0000) >> 16): average number
                                                         */
#define TSCAM_OPTION_OVEREXP_POLICY         0x68       /* [RW] Auto exposure overexposure policy: when overexposed,
...
#define TSCAM_OPTION_CDS                    0x6e       /* [RW] Correlated Double Sampling: 0~max (TSCAM_OPTION_CDS_MAX)
...
#define TSCAM_OPTION_GVCP_TIMEOUT           0x71       /* [RW] GVCP Timeout: millisecond, range = [5, 150], default: 15(wire), 30(wireless)
                                                              Unless in very special circumstances, generally no modification is required, just use the default value
                                                         */
#define TSCAM_OPTION_GVCP_RETRY             0x72       /* [RW] GVCP Retry: range = [2, 16], default: 4(wire), 8(wireless)
                                                              Unless in very special circumstances, generally no modification is required, just use the default value
                                                         */
...
#define TSCAM_OPTION_TIMED_TRIGGER_NUM      0x7c       /* [RW] Timed trigger number */
...
#define TSCAM_OPTION_LANE                   0x7e       /* [RW] Lane */
...
#define TSCAM_OPTION_BLACKLEVEL_AUTOADJUST  0x89       /* [RW] Black level automatic adjustment function: 0: off, 1: on
                                                         ...
                                                         */
#define TSCAM_OPTION_USER_SET               0x8a       /* [RW] user set */
#define TSCAM_OPTION_DIGITAL_GAIN           0x1001     /* [RW] digital gain */
#define TSCAM_OPTION_ANTI_BLOOMING          0x8b       /* [RW] Anti Blooming, maximum
                                                         ...
                                                         */
#define TSCAM_OPTION_ANTI_BLOOMING_MAX      0x8c       /* [RO] Anti Blooming */
#define TSCAM_OPTION_CDS_MAX                0x8d       /* [RO] Correlated Double Sampling */
#define TSCAM_OPTION_SCANTYPE               0x8e       /* [RW] Scan Type: 0(areascan), 1(linescan) */
#define TSCAM_OPTION_OPERATIONMODE          0x8f       /* [RW] TDI Operation Mode: 1(area), 2(TDI) */
#define TSCAM_OPTION_TDITRIGGERMODE         0x90       /* [RW] TDI Trigger Mode: 1(normal), 2(both) */
#define TSCAM_OPTION_TDISTAGE               0x91       /* [RW] TDI Trigger Stage: sensor scan stage */
#define TSCAM_OPTION_FRAMEINTERVAL          0x92       /* [RW] Frame Interval in microseconds */
#define TSCAM_OPTION_FRAMEINTERVAL_MIN      0x93       /* [RO] Frame Interval, minimum */
#define TSCAM_OPTION_FRAMEINTERVAL_MAX      0x94       /* [RO] Frame Interval, maximum */
#define TSCAM_OPTION_IMAGEPTRRAW            0x95       /* [RW] default: 0 */
#define TSCAM_OPTION_IMAGEPTRBOTH           0x96       /* [RW] default: 0 */
#define TSCAM_OPTION_LDC                    0x97       /* [RW] Low Dark Current, 0 => disable, 1 => enable */
...
#define TSCAM_PIXELFORMAT_RAW14PACK         0x14
...
TSCAM_API(int)         Tscam_get_PixelFormatBitdepth(int pixelFormat);
...
/*
 ...
 unsigned index = 0;
 do {
     if (FAILED(Tscam_get_BinningMethod(h, index, &pStrMethod)))
         break;
     ++index;
 } while (1);
*/
...
#define TSCAM_IOCONTROLTYPE_GET_PWM_FREQ                 0x19 /* PWM Frequency, range: [0, 0xffffffff] */
#define TSCAM_IOCONTROLTYPE_SET_PWM_FREQ                 0x1a
#define TSCAM_IOCONTROLTYPE_GET_PWM_DUTYRATIO            0x1b /* PWM Duty Ratio, default: 50, range: [0, 100] */
...
#define TSCAM_IOCONTROLTYPE_GET_EXPO_START_LINE          0x31 /* exposure start line, default: 0, range: [0, 16384] */
#define TSCAM_IOCONTROLTYPE_SET_EXPO_START_LINE          0x32
#define TSCAM_IOCONTROLTYPE_GET_EXPO_END_LINE            0x33 /* exposure end line, default: 0, range: [0, 16384]
...
#define TSCAM_IOCONTROLTYPE_GET_USER_PULSE_HIGH          0x3d /* User pulse high level time: us, range: [0, 0xffffffff] */
#define TSCAM_IOCONTROLTYPE_SET_USER_PULSE_HIGH          0x3e
#define TSCAM_IOCONTROLTYPE_GET_USER_PULSE_LOW           0x3f /* User pulse low level time: us, range: [0, 0xffffffff] */
#define TSCAM_IOCONTROLTYPE_SET_USER_PULSE_LOW           0x40
#define TSCAM_IOCONTROLTYPE_GET_USER_PULSE_NUMBER        0x41 /* User pulse number: default 0, range: [0, 0xffffffff] */
...
/* flash zone */
#define TSCAM_FLASH_SENSOR    0x00    /* sensor */
#define TSCAM_FLASH_USER      0x02    /* user */
/*
 action = (zone << 24) | TSCAM_FLASH_XXXX: read, write, erase, query total size, query read/write block size, query erase block size
 addr = address
 see democpp
*/
...
*/
TSCAM_API(HRESULT)  Tscam_GigeEnableV2(PTSCAM_HOTPLUG funHotPlug, void* ctxHotPlug, const char* opt);
...
TSCAM_API(HRESULT) Tscam_readPtr(HTscam h, const char* key, int len, void* pData);
TSCAM_API(HRESULT) Tscam_writePtr(HTscam h, const char* key, int len, const void* pData);
...
TSCAM_API(HRESULT) Tscam_Reset(const wchar_t* camId);
...
TSCAM_API(HRESULT) Tscam_Reset(const char* camId);
...
TSCAM_API(HRESULT)  Tscam_query_SerialNumber(const wchar_t* camId, char sn[32]);
#else
...
TSCAM_API(HRESULT)  Tscam_query_SerialNumber(const char* camId, char sn[32]);
#endif
