...
const char *ASISingleCamera::getBayerString() const
{
    return Helpers::toString(mCameraInfo.BayerPattern);
}

void ASISingleCamera::workerStreamVideo(const std::atomic_bool &isAboutToQuit)
{
    ASI_ERROR_CODE ret;
...
void ASISingleCamera::workerBlinkExposure(const std::atomic_bool &isAboutToQuit, int blinks, float duration)
{
    if (blinks <= 0)
        return;
...
void ASISingleCamera::workerExposure(const std::atomic_bool &isAboutToQuit, float duration)
{
    ASI_ERROR_CODE ret;
...
ASISingleCamera::ASISingleCamera() : CamerasListFile(GetHomeDirectory() + "/.indi/ZWOCameras.xml")
{
...
ASISingleCamera::~ASISingleCamera()
{
    if (isConnected())
        Disconnect();
}
...
bool ASISingleCamera::updateProperties()
{
...
bool ASISingleCamera::Connect()
{
    ...
    mTimerTemperature.callOnTimeout(std::bind(&ASISingleCamera::temperatureTimerTimeout, this));
...
bool ASISingleCamera::Disconnect()
{
...
void ASISingleCamera::setupParams()
{
...
bool ASISingleCamera::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    ASI_ERROR_CODE ret = ASI_SUCCESS;
...
bool ASISingleCamera::setVideoFormat(uint8_t index)
{
...
int ASISingleCamera::SetTemperature(double temperature)
{
...
bool ASISingleCamera::activateCooler(bool enable)
{
...
bool ASISingleCamera::StartExposure(float duration)
{
    mExposureRetry = 0;
    mWorker.start(std::bind(&ASISingleCamera::workerExposure, this, std::placeholders::_1, duration));
    return true;
}

bool ASISingleCamera::AbortExposure()
{
...
bool ASISingleCamera::StartStreaming()
{
#if 0
...
#endif
    mWorker.start(std::bind(&ASISingleCamera::workerStreamVideo, this, std::placeholders::_1));
    return true;
}

bool ASISingleCamera::StopStreaming()
{
    mWorker.quit();
    return true;
}

bool ASISingleCamera::UpdateCCDFrame(int x, int y, int w, int h)
{
...
bool ASISingleCamera::UpdateCCDBin(int binx, int biny)
{
    INDI_UNUSED(biny);
...
int ASISingleCamera::grabImage(float duration)
{
    ASI_ERROR_CODE ret = ASI_SUCCESS;
...
bool ASISingleCamera::isMonoBinActive()
{
    long monoBin = 0;
    ASI_BOOL isAuto = ASI_FALSE;
...
void ASISingleCamera::temperatureTimerTimeout()
{
    ASI_ERROR_CODE ret;
    ASI_BOOL isAuto = ASI_FALSE;
    long value = 0;
...
IPState ASISingleCamera::guidePulse(INDI::Timer &timer, float ms, ASI_GUIDE_DIRECTION dir)
{
    timer.stop();
...
void ASISingleCamera::stopGuidePulse(INDI::Timer &timer)
{
    if (timer.isActive())
    {
        timer.stop();
...
IPState ASISingleCamera::GuideNorth(uint32_t ms)
{
    return guidePulse(mTimerNS, ms, ASI_GUIDE_NORTH);
}

IPState ASISingleCamera::GuideSouth(uint32_t ms)
{
    return guidePulse(mTimerNS, ms, ASI_GUIDE_SOUTH);
}

IPState ASISingleCamera::GuideEast(uint32_t ms)
{
    return guidePulse(mTimerWE, ms, ASI_GUIDE_EAST);
}

IPState ASISingleCamera::GuideWest(uint32_t ms)
{
    return guidePulse(mTimerWE, ms, ASI_GUIDE_WEST);
}

void ASISingleCamera::createControls(int piNumberOfControls)
{
    ControlNP.resize(0);
    ControlSP.resize(0);
...
void ASISingleCamera::updateRecorderFormat()
{
...
void ASISingleCamera::addFITSKeywords(fitsfile *fptr, INDI::CCDChip *targetChip)
{
...
bool ASISingleCamera::saveConfigItems(FILE *fp)
{
