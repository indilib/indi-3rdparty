...
bool ToupBase::initProperties()
{
    ...
    IUFillNumberVector(&m_BlackLevelNP, &m_BlackLevelN, 1, getDeviceName(), "CCD_BLACKLEVEL", "Black Level", CONTROL_TAB, IP_RW, 60, IPS_IDLE);
    ...
    if (m_MonoCamera == false)
    {
        ...
        IUFillSwitchVector(&m_WBAutoSP, &m_WBAutoS, 1, getDeviceName(), "TC_AUTO_WB", "Auto White Balance", CONTROL_TAB, IP_RW, ISR_1OFMANY, 60, IPS_IDLE);
    }
...
void ToupBase::setupParams()
{
    ...
    if (m_maxBitDepth > 8)
    {
        FP(put_Option(m_CameraHandle, CP(OPTION_BITDEPTH), 1));// enable bitdepth
        m_BitsPerPixel = 16;
    }
    ...
    if (m_MonoCamera)// Check if mono camera
    {
        ...
        if (m_maxBitDepth > 8)
        {
            ...
            IUFillSwitch(&m_VideoFormatS[TC_VIDEO_MONO_16], "TC_VIDEO_MONO_16", (std::string("Mono ") + std::to_string(m_maxBitDepth)).c_str(), ISS_OFF);
    ...
    FP(get_Option(m_CameraHandle, CP(OPTION_CG), &conversionGain));
    ...
    {
        // Hue
        FP(get_Hue(m_CameraHandle, &nVal));
        m_ControlN[TC_HUE].value = nVal;

        // Saturation
        FP(get_Saturation(m_CameraHandle, &nVal));
        m_ControlN[TC_SATURATION].value = nVal;
    }

    // Brightness
    FP(get_Brightness(m_CameraHandle, &nVal));
    m_ControlN[TC_BRIGHTNESS].value = nVal;

    // Gamma
    FP(get_Gamma(m_CameraHandle, &nVal));
    ...
    FP(get_Option(m_CameraHandle, CP(OPTION_FRAMERATE), &frameRateLimit));
...
bool ToupBase::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && !strcmp(dev, getDeviceName()))
    {
        ...
        if (!strcmp(name, m_BlackLevelNP.name))
        {
            ...
            HRESULT rc = FP(put_Option(m_CameraHandle, CP(OPTION_BLACKLEVEL), bLevel));
            if (FAILED(rc))
            {
